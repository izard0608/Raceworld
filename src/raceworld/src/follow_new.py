...
# 直道高速与速度-转向耦合参数
FAST_SPEED_BOOST = 0.28  # 下1/2曲率接近0时的基础高速加成
FAST_MAX_ERR = 0.18  # 允许进入高速模式的最大横向误差
FAST_MAX_CURVE = 0.05  # 允许进入高速模式的最大曲率
FAST_CONTROL_CURVE_LIMIT = 0.075  # 控制曲率超过该值时禁止直道高速
STARTUP_FAST_BLOCK_FRAMES = 12  # 缓启动退出后暂时禁止高速，避免刚入正轨就冲弯
STRAIGHT_STEER_SPEED_GAIN_MIN = 0.80  # 直道最高速时PID转角倍率下限
STEER_SPEED_COUPLING = 0.28  # Scale speed down by normalized steering demand.
...
ERR_ALPHA = 0.50  # 横向误差低通滤波系数，越大越相信当前帧
...
LARGE_ERR_SPEED_LIMIT = 0.60  # 大横向误差时的最高速度
SEVERE_ERR_SPEED_LIMIT = 0.38  # 严重横向误差时的最高速度
LARGE_ERR_THRESHOLD = 0.55  # 进入大误差限速的误差阈值
SEVERE_ERR_THRESHOLD = 0.78  # 进入严重误差限速的误差阈值

STRAIGHT_PARAMS = {
    "target_speed": 0.75,  # 直道目标速度
    "min_speed": 0.20,  # 直道最低速度
    "max_steer": 0.32,  # 直道最大转角
    "deadband": 0.05,  # 直道误差死区，小于该值按0处理
    "turn_speed_drop": 0.12,  # 误差越大速度越低的降速系数
    "pid_kp": 0.82,  # 直道PID比例系数
    "pid_ki": 0.01,  # 直道PID积分系数
    "pid_kd": 0.14,  # 直道PID微分系数
    "pid_integral_limit": 0.60,  # PID积分限幅，防止积分饱和
    "steer_rate_limit": 0.12,  # 单帧转角变化限制
...
TURN_PARAMS = {
    "target_speed": 0.82,  # 弯道目标速度
    "min_speed": 0.45,  # 弯道最低速度
    "max_steer": 0.62,  # 弯道最大转角
    ...
    "steer_rate_limit": 0.20,  # 弯道单帧转角变化限制
    "sharp_turn_err": 0.55,  # 弯道大误差强制补转向阈值
    "sharp_turn_steer": 0.38,  # 弯道大误差时的最小转角
}
...
DEBUG_VERSION = "direct_line_near_curve_multiroi_v13_fast_yellow"  # 当前调试版本标识
